...
ACMD(do_cook) {
  ...
  FAILURE_CASE(!*argument, "What software do you wish to cook to a chip?");

  if (ch->in_veh) {
    FAILURE_CASE(ch->vfront, "There's not enough space up here-- try switching to the rear of the vehicle.");
    FAILURE_CASE(!ch->in_veh->flags.IsSet(VFLAG_WORKSHOP), "This vehicle doesn't have any power outlets for you to use.");
  }
  ...
  FAILURE_CASE(!cooker, "There isn't a free chip encoder here.");

  int cost = GET_PROGRAM_SIZE(chip) / 2, paid = 0;
  struct obj_data *obj = nullptr;

  FOR_ITEMS_AROUND_CH(ch, obj) {
    if (GET_OBJ_TYPE(obj) == ITEM_DECK_ACCESSORY
        && GET_DECK_ACCESSORY_TYPE(obj) == TYPE_PARTS
        && GET_DECK_ACCESSORY_PARTS_IS_CHIPS(obj)
  ...
  if (get_and_deduct_one_crafting_token_from_char(ch, "cooking")) {
...
ACMD(do_build) {
  ...
  switch (GET_PART_TYPE(obj)) {
    ...
    case PART_RAS_OVERRIDE:
        target += GET_PART_TARGET_MPCP(obj);
        duration_in_hours = GET_PART_TARGET_MPCP(obj);
        break;
    case PART_REALITY_FILTER:
        target += GET_PART_TARGET_MPCP(obj);
        duration_in_hours = GET_PART_TARGET_MPCP(obj) * 1 WORKING_DAYS;
  ...
  if (get_and_deduct_one_crafting_token_from_char(ch, "part building")) {
