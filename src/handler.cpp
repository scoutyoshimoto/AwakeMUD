...
void extract_veh(struct veh_data * veh)
{
  ...
  if (veh->prev_sub) {
    if (!veh->sub)
      mudlog("SYSERR: Extracting vehicle with prev_sub but sub=false!", NULL, LOG_SYSLOG, TRUE);
  ...
  if (veh->next_sub) {
    if (!veh->sub)
      mudlog("SYSERR: Extracting vehicle with next_sub but sub=false!", NULL, LOG_SYSLOG, TRUE);
